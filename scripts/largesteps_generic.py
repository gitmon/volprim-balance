...
class LargeSteps():
    ...
    def to_differential_3(self, c: mi.Color3f):
        """
        ...
        v_unique = dr.gather(mi.Color3f, dr.unravel(mi.Color3f, c), self.index)
        row_prod = dr.gather(mi.Color3f, v_unique, self.cols.array) * self.data.array
        u = dr.zeros(mi.Color3f, dr.width(v_unique))
        dr.scatter_reduce(dr.ReduceOp.Add, u, row_prod, self.rows.array)

        return dr.ravel(u)

    def from_differential_3(self, u):
        """
        ...
        Returns ``mitsuba.Float`:
            Vertex coordinates of the mesh.
        """
        v_unique = dr.unravel(mi.Color3f, dr.custom(SolveCholesky, self.solver, mi.TensorXf(u, (self.n_verts, 3))).array)
        return dr.ravel(dr.gather(mi.Color3f, v_unique, self.inverse))

    def to_differential_1(self, v: mi.Float):
        c = dr.repeat(v, 3)
        # Manual matrix-vector multiplication
        v_unique = dr.gather(mi.Color3f, dr.unravel(mi.Color3f, c), self.index)
        row_prod = dr.gather(mi.Color3f, v_unique, self.cols.array) * self.data.array
    ...
    def from_differential_1(self, u):
        v_unique = dr.unravel(mi.Color3f, dr.custom(SolveCholesky, self.solver, mi.TensorXf(u, (self.n_verts, 3))).array)
        return dr.gather(mi.Color3f, v_unique, self.inverse).x
